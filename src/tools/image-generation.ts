...
let imageGeneratorInstance: ImageGenerator | ResponsesImageGenerator;

if (config.apiMode === 'responses') {
  // Use new Responses API
  const responsesClient = new ResponsesAPIAdapter(config.openaiApiKey);
  imageGeneratorInstance = new ResponsesImageGenerator(
    responsesClient,
    cacheAdapter,
    optimizerAdapter,
    fileConverter,
...
export async function* generateImageWithStreaming(input: ImageGenerationInput): AsyncGenerator<any, void, unknown> {
  if (config.apiMode === 'responses' && imageGeneratorInstance instanceof ResponsesImageGenerator) {
