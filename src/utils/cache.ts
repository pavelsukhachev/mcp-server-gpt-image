...
export class ImageCache {
  ...
  private generateCacheKey(type: 'generate' | 'edit', input: ImageGenerationInput | ImageEditInput): string {
    const normalized = {
      type,
      prompt: input.prompt,
  ...
  async get(type: 'generate' | 'edit', input: ImageGenerationInput | ImageEditInput): Promise<any | null> {
  ...
  async set(type: 'generate' | 'edit', input: ImageGenerationInput | ImageEditInput, data: any, ttl?: number): Promise<void> {
    const key = this.generateCacheKey(type, input);
    ...
    try {
      ...
      if (totalSize > this.maxSize) {
        ...
        for (const stat of stats) {
          if (currentSize <= this.maxSize * 0.8) break; // Keep 80% to avoid frequent cleanups
  ...
  getCacheStats(): { memoryEntries: number; estimatedDiskUsage: string } {
    return {
