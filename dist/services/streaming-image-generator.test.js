...
describe('StreamingImageGenerator', () => {
    ...
    beforeEach(() => {
        ...
        streamingGenerator = new StreamingImageGenerator(mockOpenAIClient, mockCache, mockOptimizer);
    });
    describe('generateWithStreaming', () => {
        ...
        it('should return cached result when available', async () => {
            ...
            const events = await collectEvents(streamingGenerator.generateWithStreaming(mockInput));
        ...
        it('should emit partial images when requested', async () => {
            ...
            vi.mocked(mockCache.get).mockResolvedValue(null);
            vi.mocked(mockOpenAIClient.generateImage).mockResolvedValue(mockResponse);
            const events = await collectEvents(streamingGenerator.generateWithStreaming(inputWithPartials));
        ...
        it('should optimize images when compression is requested', async () => {
            ...
            const events = await collectEvents(streamingGenerator.generateWithStreaming(inputWithCompression));
        ...
        it('should optimize images when format is not png', async () => {
            ...
            await collectEvents(streamingGenerator.generateWithStreaming(inputWithJpeg));
            expect(mockOptimizer.optimizeBatch).toHaveBeenCalled();
        });
        it('should handle errors gracefully', async () => {
            vi.mocked(mockCache.get).mockResolvedValue(null);
            vi.mocked(mockOpenAIClient.generateImage).mockRejectedValue(new Error('API rate limit exceeded'));
            const events = await collectEvents(streamingGenerator.generateWithStreaming(mockInput));
        ...
        it('should handle auto size parameter', async () => {
            ...
            await collectEvents(streamingGenerator.generateWithStreaming(inputWithAutoSize));
        ...
        it('should log optimization results', async () => {
            ...
            await collectEvents(streamingGenerator.generateWithStreaming(inputWithOptimization));
            expect(consoleLogSpy).toHaveBeenCalledWith('Image optimized: 35.5% size reduction');
            consoleLogSpy.mockRestore();
        });
    });
});
//# sourceMappingURL=streaming-image-generator.test.js.map