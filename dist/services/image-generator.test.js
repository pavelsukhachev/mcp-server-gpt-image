...
describe('ImageGenerator', () => {
    ...
    beforeEach(() => {
        ...
        imageGenerator = new ImageGenerator(mockOpenAIClient, mockCache, mockOptimizer, mockFileConverter);
    });
    describe('generate', () => {
        const mockInput = {
        ...
        it('should handle auto size parameter', async () => {
            const inputWithAutoSize = { ...mockInput, size: 'auto' };
            vi.mocked(mockCache.get).mockResolvedValue(null);
            vi.mocked(mockOpenAIClient.generateImage).mockResolvedValue({ data: [] });
            await imageGenerator.generate(inputWithAutoSize);
            expect(mockOpenAIClient.generateImage).toHaveBeenCalledWith(expect.objectContaining({ size: '1024x1024' }));
        });
        it('should optimize images when compression is requested', async () => {
            ...
            expect(mockOptimizer.optimizeBatch).toHaveBeenCalledWith(['original-image'], inputWithCompression);
            expect(result.images).toEqual(['optimized-image']);
        });
        it('should optimize images when format is not png', async () => {
            const inputWithJpeg = { ...mockInput, format: 'jpeg' };
            const mockResponse = {
                data: [{
                        b64_json: 'original-image',
                    }],
        ...
        it('should throw error for URL response format', async () => {
            ...
            vi.mocked(mockCache.get).mockResolvedValue(null);
            vi.mocked(mockOpenAIClient.generateImage).mockResolvedValue(mockResponse);
            await expect(imageGenerator.generate(mockInput)).rejects.toThrow('URL response not supported yet. Please use b64_json.');
        });
    ...
    describe('edit', () => {
        ...
        it('should edit image when not cached', async () => {
            ...
            expect(mockFileConverter.base64ToFile).toHaveBeenCalledWith('base64-image-data', 'image_0.png', 'image/png');
        ...
        it('should handle data URL format', async () => {
            ...
            expect(mockFileConverter.extractBase64FromDataUrl).toHaveBeenCalledWith('data:image/png;base64,abc123');
        });
        it('should handle mask parameter', async () => {
            ...
            expect(mockOpenAIClient.editImage).toHaveBeenCalledWith(expect.objectContaining({
                mask: mockMaskFile,
            }));
        });
        it('should optimize edited images when requested', async () => {
            ...
            expect(mockOptimizer.optimizeBatch).toHaveBeenCalledWith(['original-edited-image'], inputWithOptimization);
            expect(result.images).toEqual(['optimized-edited-image']);
        });
    });
});
//# sourceMappingURL=image-generator.test.js.map