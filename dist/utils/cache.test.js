...
describe('ImageCache', () => {
    ...
    describe('constructor and initialization', () => {
        ...
        it('should use default values when options are not provided', () => {
            ...
            expect(fs.mkdir).toHaveBeenCalledWith(expect.stringContaining('.cache/images'), { recursive: true });
        });
    });
    describe('generateCacheKey', () => {
        it('should generate consistent keys for generation inputs', () => {
            ...
            const firstCallCount = crypto.createHash.mock.calls.length;
            vi.clearAllMocks();
            fs.stat.mockRejectedValue(new Error('Not found'));
            cache.get('generate', input);
            const secondCallCount = crypto.createHash.mock.calls.length;
        ...
        it('should generate different keys for edit inputs with images', () => {
            ...
            const hashCalls = crypto.createHash.mock.calls;
    ...
    describe('set', () => {
        ...
        it('should store data in memory and disk', async () => {
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(path.join(mockCacheDir, 'mockedhash123456.json'), JSON.stringify(mockData, null, 2));
        });
        it('should use custom TTL when provided', async () => {
            ...
            const memEntry = cache.memoryCache.get('mockedhash123456');
        ...
        it('should handle write errors gracefully', async () => {
            ...
            await cache.set('generate', mockInput, mockData);
            expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to write cache:', expect.any(Error));
            consoleErrorSpy.mockRestore();
        });
    ...
    describe('cleanupIfNeeded', () => {
        ...
        it('should remove expired entries from memory cache', async () => {
            ...
            await cache.set('generate', { prompt: 'test' }, { data: 'test' });
            expect(cache.memoryCache.has('expired')).toBe(false);
            expect(cache.memoryCache.has('valid')).toBe(true);
        });
